...
export function FindAnimalGame() {
  ...
  const [animalToFind, setAnimalToFind] = useState(
    initialAnimals[Math.floor(Math.random() * initialAnimals.length)]
  );

  const [playDog] = useSound('/audios/dog.wav');
  const [playCat] = useSound('/audios/cat.wav');

  const animalSounds = { dog: playDog, cat: playCat };
  ...
  function handleGuess(animal) {
    if (animal === animalToFind) {
      ...
      setTimeout(() => {
        if (animals.length > 1) setGameState("waiting");
  ...
  return (
    <div className="relative flex flex-col min-h-screen gap-10 justify-center items-center bg-gradient-to-b from-sky-200 to-sky-700 text-white pb-5">
      <a
        href="/game"
        className="absolute top-4 left-4 bg-white/30 text-white px-3 py-1 rounded-lg text-sm hover:bg-white/50 transition"
      >
        ← Games
      </a>
      <div className="flex flex-col gap-3 justify-center items-center h-[10%] my-10">
        ...
        {gameState === "completed" && (
          <div className="text-3xl font-bold text-center text-slate-900">
            ...
            <button onClick={resetGame} className="bg-blue-600 px-4 py-2 rounded-lg text-white mt-5 hover:bg-blue-500 block mx-auto">
              Play Again
            </button>
          </div>
        )}
